  ...
  return (
    <div className="flex flex-col space-y-6">
      <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="rounded-md border p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium">{recentActivities[0]?.timestamp || "N/A"}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <DashboardCard title="Quick Actions" className="border-l-4 border-l-slate-700">
        ...
        <DashboardCard title="My Recent Activities" className="border-l-4 border-l-slate-700">
          <div className="max-h-60 overflow-y-auto pr-2">
            {recentActivities.length > 0 ? (
              <div className="space-y-3">
                {recentActivities.map((activity, index) => (
                  <div key={index} className="rounded-md border p-3 hover:bg-slate-50">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium">
                          {activity.type === "created" ? "Created" : "Updated"} profile for{" "}
                          <Link href={`/client/${activity.clientId}`} className="text-blue-600 hover:underline">
                            {activity.clientName}
                          </Link>
                        </p>
...
function DefaultDashboard() {
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      <DashboardCard title="Recent Contacts" className="border-l-4 border-l-slate-700">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-slate-500">You have no recent contacts</p>
